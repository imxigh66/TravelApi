...
{
    public class GetUserTripsQueryHandler : IRequestHandler<GetUserTripsQuery, List<TripDto>>
    {
        private readonly IApplicationDbContext _context;
        public GetUserTripsQueryHandler(IApplicationDbContext context)
        {
            _context = context;
        }
        public async Task<List<TripDto>> Handle(GetUserTripsQuery request, CancellationToken cancellationToken)
        {
            var trips = await _context.Trips
        .Where(t => t.OwnerId == request.UserId && t.IsPublic)
