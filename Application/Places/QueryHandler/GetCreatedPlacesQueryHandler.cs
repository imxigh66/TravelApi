...
{
    public class GetCreatedPlacesQueryHandler : IRequestHandler<GetCreatedPlacesQuery, PaginatedList<PlaceDto>>
    {
        private readonly IApplicationDbContext _context;
        public GetCreatedPlacesQueryHandler(IApplicationDbContext context)
        {
            _context = context;
        }
        public async Task<PaginatedList<PlaceDto>> Handle(GetCreatedPlacesQuery request, CancellationToken cancellationToken)
        {
            var query = _context.Places
               .AsNoTracking()
               .Where(p => p.CreatedBy == request.UserId)
